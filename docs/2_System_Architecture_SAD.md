...
- **编程语言**：Python 3.10+
- **流程调度与提示词管理**：[DSPy](https://dspy-docs.vercel.app/)（用于声明式编程与复杂的 Prompt 编排）
...
- **`core/state_manager.py`**：【新增核心枢纽】项目状态机与断点续传模块。它是整个工作流的大脑，负责扫描 `outputs/` 目录，通过文件探针（探测当前项目最高阶段生成物）推断现有项目的进度阶段；并在关键并发节点（如阶段二）记录 `.cursor` 游标卡片，以确保在意外中断后能够精准完成反序列化与断点重启（**Resume from Breakpoint**）。
- **`core/llm_client.py`**：极其关键的底层网关基础设施。所有的并发阅读调用都在此阻塞与调度，以防触发封禁，或者在此封装自动 fallback 机制。
...
4. **状态推断 (State Inference)**：通过探查各子文件夹内的文件清单，反推当前项目处于第几阶段。例如：
   ...
   - 存在 `3_outline_matrix.json` -> 系统提示：“当前处于【阶段四：撰写初稿】，是否继续？”
