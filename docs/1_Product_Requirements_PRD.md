...
系统为纯命令行（CLI）交互界面，避免图形化干扰，所有的中间产物以明确的序号存放在独立的 `outputs/` 项目文件夹下。为了提供极客且流畅的无状态交互体验，系统必须实现基于文件系统探针的智能工作流管理：
...
- **Agent角色**：选题顾问智能体 (Topic Advisor Agent)。调用外部学术文献 API 梳理前沿数据。
