...
### [输出] 2_final_corpus.yaml (核心总库)

**明确定义**：`2_final_corpus.yaml` 在物理和逻辑上，就是 `2_consensus_data.yaml`（无分歧共识档案）与 `2_llm3_verified.yaml`（存在争议但经过第三方大模型重裁后的有效档案）这**两个文件的直接全量合并**，作为最后沉淀出的核心 RAG 底料。
...
- **底层支撑**：第二阶段收口输出的 `2_final_corpus.yaml`，提供全部确定相关且评级可靠的实体史料阵列。

---

### [输出] 3_outline_matrix.json

系统将原始需求中的大纲文档结构化为 JSON 契约，以彻底锁死“论点与史料（piece_id）的映射关系”，拒绝空泛论证。此文件也是阶段四撰写初稿的导航图。

- **核心契约要求**：最底层的 `evidence_anchors` 必须仅能包含 `2_final_corpus.yaml` 中客观存在的合法 `piece_id`，不允许虚构不存在的 ID。
...
### [输入] JSON 论纲契约与史料库左连接 (Data JOIN)

- **逻辑图纸**：第三阶段产出的 `3_outline_matrix.json`（骨干与 `piece_id` 的绑定树）。
- **内容抽屉**：第二阶段产出的 `2_final_corpus.yaml`（根据 `piece_id` 提取对应的 `original_text`）。
- **装配要求**：该阶段执行脚本必须在内存中，提取 JSON 里的 `evidence_anchors` 数组，并去 YAML 字典中完成 JOIN（左连接）操作。将原始史料文本与对应的大纲逻辑节点拼装绑定后，一并送入 Writer Agent 进行生成，以此彻底断绝大模型的史料幻觉。

---

### [输出] 4_first_draft.md

接收阶段三的 JSON 大纲和阶段二的史料 YAML，运行带有检索增强生成（RAG）限制的写手模型，输出学术论文初稿文本。
...
- **防篡改对照组**：包含原文和出处的 `2_final_corpus.yaml`，用于复查 Writer Agent 是否在行文中私自篡改了原始史料字符。
